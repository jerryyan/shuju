...
class Lls_jinjia extends Controller
{
    ...
    public function jjImport()
    {
        if (request()->isPost()) {
            ...
            if ($info) {
                ...
                $file_data = ['name' => $file_name, 'atime' => $time, 'source' => $source];
                //获取上传的文件id
                $jid = Db::name('ls_jjfile')->insertGetId($file_data);
                ...
                if ($extension == 'xlsx' || $extension == 'xls') {
                    ...
                    if ($source == '百度') {
                        ...
                        for ($i = 2; $i <= $highestRow; $i++) {
                            ...
                            $data[$a]['xiangmu'] = $xiangmu;
                            $data[$a]['source'] = $source;
                            $data[$a]['jihua'] = $objPHPExcel->getActiveSheet()->getCell("D" . $i)->getValue();
                            $data[$a]['dy'] = $objPHPExcel->getActiveSheet()->getCell("C" . $i)->getValue();
                            $data[$a]['keyword'] = $objPHPExcel->getActiveSheet()->getCell("A" . $i)->getValue();
                            $data[$a]['ppms'] = $objPHPExcel->getActiveSheet()->getCell("R" . $i)->getValue();
                            $chujia = $objPHPExcel->getActiveSheet()->getCell("H" . $i)->getValue();
                            $data[$a]['chujia'] = $this->findNum($chujia);
                            $data[$a]['pweb'] = $objPHPExcel->getActiveSheet()->getCell("P" . $i)->getValue();
                            $data[$a]['mweb'] = $objPHPExcel->getActiveSheet()->getCell("Q" . $i)->getValue();
                            $data[$a]['zmxf'] = $objPHPExcel->getActiveSheet()->getCell("W" . $i)->getValue();
                            $data[$a]['click'] = $objPHPExcel->getActiveSheet()->getCell("T" . $i)->getValue();
                ...
                // halt($data);
                //往数据库添加数据
                $res = Db::name('ls_jinjia')->insertAll($data);
                unset($data);
                if ($res) {
    ...
    public function jjFile()
    {
        ...
        $count = db('ls_jjfile')->count();
        //每页显示条数
        $limit = 10;
        $index = ($p - 1) * $limit;
        $data = db('ls_jjfile')->limit($index, $limit)->order('id', 'desc')->select();
        $this->assign('p', $p);
    ...
    {
        $id = Request::instance()->post('id');
        $res = Db::table('ls_jjfile')->delete($id);
        Db::table('ls_jinjia')->where('jid', $id)->delete();
        if ($res) {
            return json(['code' => 1, 'msg' => '删除成功!']);
        } else {
            return json(['code' => 0, 'msg' => '删除失败!']);
    ...
    {
        if (stripos($url, 'source=') !== false && stripos($url, 'source=bd') !== false) {
            // $url = str_replace("?&", "?", $url);
            // $arr = parse_url($url);           
