...
class Search extends Controller
{
    public function index()
    {
        ...
        foreach ($swt as $k => $v) {
            ...
            $items = array_filter($jinjia, ,ARRAY_FILTER_USE_BOTH);
            halt($items);
            if (count($items) > 0) {
                $data[$k]['zhanghu'] = $items[0]['zhanghu'];
                $data[$k]['jihua'] = $items[0]['jihua'];
                $data[$k]['dy'] = $items[0]['dy'];
                $data[$k]['tfword'] = $items[0]['keyword'];
            }
            halt($data);
        }
    }
    function aa($v) use ($fwwz_utm) {
