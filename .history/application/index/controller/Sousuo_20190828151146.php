    ...
    public function swtImport()
    {
        if (request()->isPost()) {
            ...
            if ($info) {
                ...
                // foreach ($search as $k => $v) {
                //     $cigen_arr[$k][$v['guize']] = $v['cigen'];
                //     $cigen_arr[$k][$v['guize']] = $v['xiangmu'];
                // }
                ...
                if ($source == '百度') {
                    for ($i = 9; $i <= $highestRow; $i++) {
                        ...
                        foreach ($search as $vv['cigen'] => $vv) {
                            if ($xiangmu == $vv['xiangmu']) {
                                if (stripos($vv['cigen'], '*') !== false) {
                                    $vv['cigen']_arr = explode('*', $vv['cigen']);
                                    if (stripos($ssword, $vv['cigen']_arr[0]) !== false && stripos($ssword, $vv['cigen']_arr[1]) !== false) {
                                        $cigen = $vv;
                                    }
                                } else {
                                    if (stripos($ssword, $vv['cigen']) !== false) {
                                        $cigen = $vv;
                                    }
                                }
                            }
                    ...
                    for ($i = 2; $i <= $highestRow; $i++) {
                        ...
                        $data[$a]['sid'] = $sid;
                        $data[$a]['source'] = $source;
                        foreach ($cigen_arr as $vv['cigen'] => $vv) {
                            if (stripos($vv['cigen'], '*') !== false) {
                                $vv['cigen']_arr = explode('*', $vv['cigen']);
                                if (stripos($ssword, $vv['cigen']_arr[0]) !== false && stripos($ssword, $vv['cigen']_arr[1]) !== false) {
                                    $cigen = $vv;
