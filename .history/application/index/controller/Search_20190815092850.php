    ...
    public function index()
    {
        if (request()->isPost()) {
           ...
           // return  json_encode($data);
            $g_sql = "SELECT cigen,ssword,sum(click) as aclick,sum(zx) as azx,round(sum(zmxf),2) as azmxf,SUM(fanwwen) as afw,SUM(duihua) as adh,SUM(youxiao) as ayx,SUM(liulian) as aliulian FROM search_info  where xiangmu='远大肛肠' and atime >=? and atime <=? GROUP BY cigen  ORDER BY azmxf DESC";
