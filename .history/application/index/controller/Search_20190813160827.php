...
class Search extends Controller
{
    public function index()
    {
        if (request()->isPost()) {
            ...
            $a_sql = "SELECT cigen,sum(click) as aclick,sum(zx) as azx,round(sum(zmxf),2) as azmxf,SUM(fanwwen) as afw,SUM(duihua) as adh,SUM(youxiao) as ayx,SUM(liulian) as aliulian FROM search_info where atime >='{$}' and atime <='{$endDate}'  GROUP BY cigen ORDER BY azmxf  DESC";
