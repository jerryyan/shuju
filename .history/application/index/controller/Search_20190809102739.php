...
class Search extends Controller
{
    public function index()
    {
        ...
        $m_data = db::query("select *,a.keyword as ssword from swt as a  LEFT  JOIN  jinjia as b ON a.utm=b.mutm where  fwwz like '%utm_source=bdm%' GROUP BY a.id");
        $p_data = db::query("select *,a.keyword as ssword from swt as a  LEFT  JOIN  jinjia as b ON a.utm=b.putm where  fwwz like '%utm_source=bdp%' GROUP BY a.id");
        ...
        foreach ($list as $k => $v) {
            ...
            foreach ($cigen_arr as $a => $b) {
                if (stripos($a, '*') !== false) {
                    $a_arr = explode('*', $a);
                    if (stripos($v['ssword'], $a_arr[0]) !== false && stripos($v['keyword'], $a_arr[1]) !== false) {
                        $data[$k]['cigen'] = $b;
                    }
                } else {
