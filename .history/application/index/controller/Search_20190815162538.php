...
class Search extends Controller
{
    ...
    public function exportCsv()
    {
       // set_time_limit(0);  //防止请求超时   
        ...
        $list = db::query("select * from search_info where atime >=? and atime <=? limit 0,3000", [$start, $end]);
        // $list = db::query("select * from search_info ");
        $totalData = count($list); //从数据库获取总量，假设是100w
        ...
        for ($i = 1; $i <= $pages; $i++) {
            ...
            // $xlsdata = ob_get_contents();
            // ob_end_clean();
            // return ['filename' => $csvFileName, 'file' => "data:application/vnd.ms-excel;base64," . base64_encode($xlsdata)];
        }
    }
