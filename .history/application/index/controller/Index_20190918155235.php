...
class Index extends Controller
{
        ...
        if (request()->isPost()) {
            ...
            if ($info) {
                ...
                if ($res) {
                    ...
                    $file_data = ['name' => $file_name, 'atime' => $time, 'uniqid' => $uniqid];
                    Db::name('swt_file')->insert($file_data);
    ...
    public function swtFile()
    {
        ...
        if (empty($str)) {
            ...
            $data = db('swt_file')->where('name', 'like', "%$str%")->limit($index, $limit)->order('id', 'desc')->select();
            $count = db('swt_file')->where('name', 'like', "%$str%")->count();
        }
        $this->assign('p', $p);
    ...
    public function del()
    {
        ...
        $res = Db::table('swt_file')->where('uniqid', $uniqid)->delete();
        Db::table('swt')->where('uniqid', $uniqid)->delete();
        if ($res) {
