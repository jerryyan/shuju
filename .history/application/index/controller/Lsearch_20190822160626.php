...
class Lsearch extends Controller
{
    public function index()
    {
        if (request()->isPost()) {
            ...
            $x_sql = "SELECT xiangmu,cigen,sum(click) as aclick,sum(zx) as azx,round(sum(zmxf),2) as azmxf,SUM(fanwwen) as afw,SUM(duihua) as adh,SUM(youxiao) as ayx,SUM(liulian) as aliulian FROM ls_info  WHERE $xm and atime >=? and atime <=? GROUP BY xiangmu ORDER BY azmxf  DESC";
            ...
            foreach ($all_list as $k => $v) {
                ...
                $data[$k]['dhl'] = $this->getRound($v['adh'], $v['afw'], 1);
            ...
            $g_sql = "SELECT xiangmu,cigen,ssword,sum(click) as aclick,sum(zx) as azx,round(sum(zmxf),2) as azmxf,SUM(fanwwen) as afw,SUM(duihua) as adh,SUM(youxiao) as ayx,SUM(liulian) as aliulian FROM ls_info  WHERE xiangmu='远大肛肠' and atime >=? and atime <=? GROUP BY cigen ORDER BY azmxf  DESC";
            ...
            $w_sql = "SELECT xiangmu,cigen,ssword,sum(click) as aclick,sum(zx) as azx,round(sum(zmxf),2) as azmxf,SUM(fanwwen) as afw,SUM(duihua) as adh,SUM(youxiao) as ayx,SUM(liulian) as aliulian FROM ls_info  WHERE xiangmu='远大胃肠' and atime >=? and atime <=? GROUP BY cigen ORDER BY azmxf  DESC";
            ...
            foreach ($w_list as $k => $v) {
                ...
                $wc_list[$k]['dhl'] = $this->getRound($v['adh'], $v['afw'], 1);
            ...
            if ($xiangmu == 1) {
                ...
                $alist = array_merge($data, $wc_list);
            } else {
    ...
    public function export()
    {
        if (request()->isPost()) {
            ...
            $start = Request::instance()->post('startDate');
            $end = Request::instance()->post('endDate');
            $list = db::query("select * from ls_info where  xiangmu!='' and atime >=? and atime <=?", [$start, $end]);
    ...
    public function exportCsv()
    {
        ...
        $list = db::query("select * from ls_info where atime >=? and atime <=?", [$startDate, $endDate]);
        foreach ($list as $k => $v) {
            $rowData[$k] = [
                $v['atime'],
                $v['stype'],
