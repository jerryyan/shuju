...
class Search extends Controller
{
    ...
    {
        $a_sql = 'SELECT cigen,sum(click) as aclick,sum(zx) as azx,round(sum(zmxf),2) as azmxf,SUM(fanwwen) as afw,SUM(duihua) as adh,SUM(youxiao) as ayx,SUM(liulian) as aliulian FROM search_info  GROUP BY cigen';
        ...
        foreach ($all_list as $k => $v) {
            ...
            $data[$k]['yxzb'] = $this->getRound($v['ayd'], $v['adh'], 1);
            ...
            $data[$k]['yxcb'] = $this->getRound($v['azmxf'], $v['ayd']);
            $data[$k]['llxb'] = $this->getRound($v['azmxf'], $v['aliulian']);
        }
        $w_sql = "SELECT cigen,ssword,sum(click) as aclick,sum(zx) as azx,round(sum(zmxf),2) as azmxf,SUM(fanwwen) as afw,SUM(duihua) as adh,SUM(youxiao) as ayx,SUM(liulian) as aliulian FROM search_info  GROUP BY ssword";
        ...
        foreach ($all_list as $k => $v) {
            ...
            $list[$k]['yxcb'] = $this->getRound($v['azmxf'], $v['ayd']);
