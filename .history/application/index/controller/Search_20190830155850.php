...
class Search extends Controller
{
    public function index()
    {
        if (request()->isPost()) {
            ...
            $x_sql = "SELECT xiangmu,cigen,sum(click) as aclick,sum(zx) as azx,round(sum(zmxf),2) as azmxf,SUM(fanwwen) as afw,SUM(duihua) as adh,SUM(youxiao) as ayx,SUM(liulian) as aliulian FROM search_info  WHERE $xm and atime >=? and atime <=? GROUP BY xiangmu ORDER BY azmxf  DESC";
    ...
    public function exportCsv()
    {
        ...
        if ($type == 1) {
            $where = " and xiangmu ='远大肛肠'";
        } elseif ($type == 2) {
            $where = " and xiangmu ='远大胃肠'";
        } else {
            $where = " and 1=1";
        }
        $list = db::query("select * from search_info where atime >=? and atime <=? $where", [$startDate, $endDate]);
        foreach ($list as $k => $v) {
            $rowData[$k] = [
                $v['atime'],
                $v['stype'],
