...
class Search extends Controller
{
    public function index()
    {
        ...
        $swt = db::query("select * from swt as a  LEFT  JOIN  jinjia as b ON a.utm=b.mutm where  fwwz like '%utm_source=bdm%' GROUP BY a.id");
        // $jinjia = db::query("select * from jinjia where pweb like '%utm_source=bd%' or mweb like '%utm_source=bd%'");
        foreach ($swt as $k => $v) {
            $fwwz_utm = $this->getUtm($v['fwwz']);
            $jinjia = db::query("select * from jinjia where pweb like '%$fwwz_utm%' or mweb like '%$fwwz_utm%'");
            halt($jinjia);
            // $data[$k] = $v;
