...
class Sousuo extends Controller
{
    ...
    public function show()
    {
        ...
        if (empty($str)) {
            ...
            $data = db('jj_file')->where('name', 'like', "%$str%")->limit($index, $limit)->order('id', 'desc')->select();
            $count = db('jj_file')->where('name', 'like', "%$str%")->count();
            $data = db('ss_file')->where('name', 'like', "%$str%")->limit($index, $limit)->order('id', 'desc')->select();
            $count = db('ss_file')->where('name', 'like', "%$str%")->count();
        }
        ...
        if (request()->isPost()) {
            ...
            if ($info) {
                ...
                if ($extension == 'xlsx') {
                    ...
                    //默认输入字符集
                    //$encoding = mb_detect_encoding(file_get_contents($path), 'ASCII,UTF-8,GBK,GB2312,GB18030');
                    $PHPReader->setInputEncoding('GB18030');
                    //默认的分隔符+3
                ...
                if ($res) {
                    //添加上传文件记录到数据库
                    $time = date('Y-m-d H:i:s');
                    $file_data = ['xiangmu' => $xiangmu, 'source' => $source, 'name' => $file_name, 'atime' => $time, 'uniqid' => $uniqid];
                    $uniqid = Db::name('ss_file')->insert($file_data);
