...
class Lls_swt extends Controller
{
    ...
    public function ls_swtImport()
    {
        if (request()->isPost()) {
            ...
            if ($info) {
                ...
                //获取上传的文件id
                $fid = Db::name('ls_swt_file')->insertGetId($file_data);
                ...
                for ($i = 4; $i <= $highestRow; $i++) {
                    ...
                    $res = Db::name('ls_swt')->insert($data);
                    // unset($data); //销毁插入的数据数组

                }
                // 往数据库添加数据
                // $res = Db::name('ls_swt')->insertAll($data);
                // $res = Db::name('ls_swt')->fetchSql()->insertAll($data);
    ...
    public function ls_swtFile()
    {
        $p = Request::instance()->get('p');
        if (empty($p)) {
            $p = 1;
        }
        $count = db('ls_swt_file')->count();
        //每页显示条数
        $limit = 10;
        $index = ($p - 1) * $limit;
        $data = db('ls_swt_file')->limit($index, $limit)->select();
        $this->assign('p', $p);
    ...
    public function del()
    {
        ...
        $res = Db::table('ls_swt_file')->delete($id);
        Db::table('ls_swt')->where('fid', $id)->delete();
        if ($res) {
    ...
    {
        if (stripos($url, 'utm_source=') !== false && stripos($url, 'utm_source=bd') !== false) {
            // $url = str_replace("?&", "?", $url);
            // $arr = parse_url($url); 
