...
class Search extends Controller
{
    public function index()
    {
        if (request()->isPost()) {
            ...
            $w_sql = "SELECT xiangmu,cigen,ssword,sum(click) as aclick,sum(zx) as azx,round(sum(zmxf),2) as azmxf,SUM(fanwwen) as afw,SUM(duihua) as adh,SUM(youxiao) as ayx,SUM(liulian) as aliulian FROM search_info  where cigen!='其他'  and atime >=? and atime <=? GROUP BY cigen ORDER BY azmxf  DESC";
