...
class Sousuo extends Controller
{
    ...
    public function swtData()
    {
        ...
        $jinjia = db::query("select * from jinjia where left(stime,10)='{$timer}'");
        if (count($jinjia) < 1) {
            return json(['code' => 0, 'msg' => "请上传{$timer}的竞价数据!"]);
        }

        $info = db::query("select * from search_info where stype='swt' and left(stime,10)='{$timer}'");
        if (count($info) >= 1) {
            return json(['code' => 0, 'msg' => "请先删除{$timer}已经的数据!"]);
        }
