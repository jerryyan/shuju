...
class Search extends Controller
{
    ...
    {
        $sql = 'SELECT sum(click) as aclick,sum(zx) as azx,round(sum(zmxf),2) as azmxf,SUM(fanwwen) as afw,SUM(duihua) as adh,SUM(youxiao) as ayd,SUM(liulian) as aliulian FROM search_info  GROUP BY cigen';
        ...
        foreach ($all_list as $k => $v) {
            ...
            $data[$k]['fanwen'] = $v['fanwwen'];
            $data[$k]['duihua'] = $v['afw'];
            $data[$k]['youxiao'] = $v['cigen'];
            $data[$k]['liulian'] = $v['cigen'];
            $data[$k]['djl'] = $v['cigen'];
            $data[$k]['ddl'] = $v['cigen'];
            $data[$k]['yxzb'] = $v['cigen'];
            $data[$k]['llv'] = $v['cigen'];
