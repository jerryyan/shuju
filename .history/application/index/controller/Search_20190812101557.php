...
class Search extends Controller
{
    ...
    {
        //$sql='SELECT sum(click) as aclick,sum(zx) as azx,sum(zmxf) as azmxf,SUM(fanwwen) as afw,SUM(duihua) as adh,SUM(youxiao) as yd,SUM(liulian) as aliulian FROM search_info GROUP BY cigen';
        $list = db('search_info')->order('id', 'desc')->select();

        $sum = 0;
        foreach ($list as $k => $k) {
            $sum = (int)$kv['click'] + $sum;
        }
        halt($sum);
        return view();
    }
