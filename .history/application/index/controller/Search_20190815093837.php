...
class Search extends Controller
{
    public function index()
    {
        if (request()->isPost()) {
            ...
            $w_sql = "SELECT xiangmu,cigen,ssword,sum(click) as aclick,sum(zx) as azx,round(sum(zmxf),2) as azmxf,SUM(fanwwen) as afw,SUM(duihua) as adh,SUM(youxiao) as ayx,SUM(liulian) as aliulian FROM search_info  WHERE xiangmu='远大肛肠' and atime >=? and atime <=? GROUP BY cigen ORDER BY azmxf  DESC";
            $all_list = db::query($w_sql, [$startDate, $endDate]);
            foreach ($all_list as $k => $v) {
                $list[$k]['id'] = $v['cigen'];
                $list[$k]['pid'] = $v['xiangmu'];
    ...
    public function export()
    {
        if (request()->isPost()) {
            ...
            $outfile = $start . '到' . $end . "搜索词数据.xlsx";
