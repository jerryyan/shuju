...
class Search extends Controller
{
    public function index()
    {
        ...
        // halt($cigen_arr);
        $swt = db::query("select * from swt where  fwwz like '%utm_source=bd%' and keyword like '%便血%'");
        // foreach ($swt as $k => $v) {
        ...
        foreach ($swt as $k => $v) {
            ...
            foreach ($cigen_arr as $a => $b) {
                if (stripos($a, '*')!== false) {
                    $a_arr = explode('*', $a);
                    if (stripos($v['keyword'], $a_arr[0])!== false && stripos($v['keyword'], $a_arr[1])) {
                        $data[$k]['cigen'] = $b;
                    } else {
                        $data[$k]['cigen'] = '其他';
                    }
                } else {
                    if (stripos($v['keyword'], $a)) {
                        $data[$k]['cigen'] = $b;
                    } else {
                        $data[$k]['cigen'] = '其他';
                    }
