...
class Search extends Controller
{
    public function index()
    {
        if (request()->isPost()) {
            ...
            $x_sql="SELECT xiangmu,cigen,sum(click) as aclick,sum(zx) as azx,round(sum(zmxf),2) as azmxf,SUM(fanwwen) as afw,SUM(duihua) as adh,SUM(youxiao) as ayx,SUM(liulian) as aliulian FROM search_info  WHERE xiangmu !='' GROUP BY xiangmu ORDER BY azmxf  DESC";
            ...
            $all_list = db::query($a_sql, [$startDate, $endDate]);
            $data = array();
