...
class Lswt extends Controller
{
    ...
    public function swtImport()
    {
        if (request()->isPost()) {
            ...
            if ($info) {
                ...
                //获取上传的文件id
                $fid = Db::name('ls_swtfile')->insertGetId($file_data);
    ...
    public function swtFile()
    {
        ...
        $count = db('ls_swtfile')->count();
        //每页显示条数
        $limit = 10;
        $index = ($p - 1) * $limit;
        $data = db('ls_swtfile')->limit($index, $limit)->select();
        $this->assign('p', $p);
    ...
    public function del()
    {
        ...
        $res = Db::table('ls_swtfile')->delete($id);
