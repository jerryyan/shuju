...
class Lsousuo extends Controller
{
    public function index()
    {
        ...
        // return $this->fetch();
        $data = db('ls_ssfile')->order('id', 'desc')->select();
        $this->assign('data', $data);
        return $this->fetch();
    }
    ...
    public function swtData()
    {
        ...
        // halt($data);
        $res = Db::name('ls_info')->insertAll($data);
        if ($res) {
    ...
    public function swtImport()
    {
        if (request()->isPost()) {
            ...
            if ($info) {
                ...
                //获取上传的文件id
                $sid = Db::name('ls_ssfile')->insertGetId($file_data);
                if ($source == '搜狗') {
    ...
    public function del()
    {
        ...
        $res = Db::table('ls_ssfile')->delete($id);
        Db::table('ls_info')->where('sid', $id)->delete();
        if ($res) {
